...
const pedido = ["Mario", "papas", "helado"];
...
function findJavaScript(matrix) {
  for (i = 0; i < matrix.length; i++) {
    const indice = matrix[i].indexOf("JavaScript");
    if (indice !== -1) {
      return [i, indice];
    }
  }
...
function findMinMaxPages(books) {
  ...
  for (i = 1; i < books.length; i++) {
    if (books[i] < books[indiceMin]) {
      indiceMin = i;
    }
    if (books[i] > books[inidiceMax]) {
      inidiceMax = i;
    }
  }
  return [indiceMin, inidiceMax];
}
